...
PROGRAM="SIG-E-SHADOW-DETECTOR3-OBSLEDGER1"
...
DETECTOR=Path("runtime/sig_e/shadow_detector_eurusd_london_pdlow_trap_long_current.json")
STATE=Path("state/sig_e_shadow_detector_observation/eurusd_london_pdlow_trap_long_obsledger_v1.json")
RUNTIME=Path("runtime/sig_e/shadow_detector_eurusd_london_pdlow_trap_long_obsledger_current.json")
PANEL=Path("panel/brain4/sig_e_shadow_detector3_obsledger_status_current.json")
OUT=Path("outputs/_sig_e_shadow_detector3_obsledger/sig_e_shadow_detector3_obsledger_build_result.json")
...
BOUND=["OBSERVATION_LEDGER_ONLY","SHADOW_RESEARCH_ONLY","NOT_SIGNAL","NO_TRADE_PROPOSAL","NO_ENTRY_STOP_TARGET","NO_RISK_OR_POSITION_SIZING","NO_BROKER_EXECUTION","NO_AUTO_EXECUTION","NO_MEMORY_PROMOTION","NO_RULE_REWRITE"]
def now(): return datetime.utcnow().replace(microsecond=0).isoformat()+"Z"
def obs_id():
    gh=os.environ.get("GITHUB_RUN_ID"); att=os.environ.get("GITHUB_RUN_ATTEMPT","1")
    return f"GH_{gh}_{att}" if gh else "LOCAL_"+datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
def load(p):
...
def state():
    s=load(STATE)
    if not isinstance(s,dict): s={"state_version":"sig_e_shadow_detector3_obsledger_v1","program":PROGRAM,"created_utc":now(),"detector_id":DETECTOR_ID,"source_spec_id":SOURCE_SPEC_ID,"refresh_records":[],"near_miss_records":[],"shadow_events":[],"outcome_records":[]}
...
def compact(d):
    r=reg(d); oid=obs_id()
    return {"observation_run_id":oid,"record_id":oid+"|"+str(d.get("detector_run_id")),"detector_id":DETECTOR_ID,"source_spec_id":SOURCE_SPEC_ID,"detector_run_id":d.get("detector_run_id"),"created_utc":now(),"detector_created_utc":d.get("created_utc"),"detector_status":d.get("detector_status"),"status_reason":d.get("status_reason"),"is_shadow_match":d.get("is_shadow_match") is True,"instrument":d.get("instrument"),"direction":d.get("direction"),"session_bucket":r.get("session_bucket"),"d1_trend_state":r.get("d1_trend_state"),"h4_trend_state":r.get("h4_trend_state"),"htf_alignment":r.get("htf_alignment"),"volatility_state_or_d1_vol_bucket":r.get("volatility_state_or_d1_vol_bucket"),"range_state":r.get("range_state"),"tradeability_context":r.get("tradeability_context"),"h1_bar_open_ts_utc":(d.get("surface_snapshot") or {}).get("h1_bar_open_ts_utc"),"m15_bar_open_ts_utc":(d.get("surface_snapshot") or {}).get("m15_bar_open_ts_utc"),"setup_h1_open_ts_utc":d.get("setup_h1_open_ts_utc"),"trigger_h1_open_ts_utc":d.get("trigger_h1_open_ts_utc"),"m15_confirm_ts_utc":d.get("m15_confirm_ts_utc"),"shadow_event_id":d.get("shadow_event_id")}
def nscore(s): return {"REGIME_NOT_MATCHED":1,"REFERENCE_LEVEL_UNAVAILABLE":1,"SETUP_NOT_FORMED":2,"H1_TRIGGER_WAIT":3,"H1_TRIGGER_NOT_CONFIRMED":3,"M15_TRIGGER_WAIT":4,"SHADOW_MATCH_CONFIRMED":5}.get(s,0)
def main():
    d=load(DETECTOR); s=state()
    if not isinstance(d,dict):
        cur={"program":PROGRAM,"created_utc":now(),"ledger_status":"BLOCKED","reason":"detector current missing","detector_id":DETECTOR_ID,"source_spec_id":SOURCE_SPEC_ID,"authority":AUTH,"boundary":BOUND}; write(RUNTIME,cur); write(PANEL,cur); write(OUT,cur); return
    r=compact(d)
    if r["record_id"] not in {x.get("record_id") for x in s["refresh_records"] if isinstance(x,dict)}: s["refresh_records"].append(r)
    near={"REGIME_NOT_MATCHED","REFERENCE_LEVEL_UNAVAILABLE","SETUP_NOT_FORMED","H1_TRIGGER_WAIT","H1_TRIGGER_NOT_CONFIRMED","M15_TRIGGER_WAIT","SHADOW_MATCH_CONFIRMED"}
    if r.get("detector_status") in near:
        k=r["record_id"]+"|"+str(r.get("detector_status"))
        ex={str(x.get("record_id"))+"|"+str(x.get("detector_status")) for x in s["near_miss_records"] if isinstance(x,dict)}
        if k not in ex:
            n=dict(r); n["near_miss_score"]=nscore(r.get("detector_status")); s["near_miss_records"].append(n)
    if d.get("detector_status")=="SHADOW_MATCH_CONFIRMED" and d.get("shadow_event_id"):
        if d.get("shadow_event_id") not in {e.get("shadow_event_id") for e in s["shadow_events"] if isinstance(e,dict)}:
            s["shadow_events"].append({"detector_id":DETECTOR_ID,"source_spec_id":SOURCE_SPEC_ID,"shadow_event_id":d.get("shadow_event_id"),"created_utc":d.get("created_utc"),"instrument":"EURUSD","direction":"LONG","detector_status":d.get("detector_status"),"setup_h1_open_ts_utc":d.get("setup_h1_open_ts_utc"),"trigger_h1_open_ts_utc":d.get("trigger_h1_open_ts_utc"),"observation_horizon_h1_bars":12,"outcome_status":"PENDING","authority":AUTH,"boundary":BOUND})
    ...
    print("SIG_E_SHADOW_DETECTOR3_OBSLEDGER_DONE")
