...
TOKEN = "scripts/run_shadow_outcome_01d.py"
reports = []
...
for wf in WORKFLOWS:
    ...
    patched = False
    for anchor in [
        "scripts/run_shadow_outcome_01c_live_recent_price_bridge.py",
    ...
    if not patched:
        ...
        report["note"] = "patched_after_outcome_bridge_step"

    wf.with_suffix(wf.suffix + ".bak_shadow_outcome_01d").write_text(original, encoding="utf-8")
    wf.write_text(text, encoding="utf-8")
    report["patched"] = True
    reports.append(report)

out = ROOT / "proofs/shadow_outcome_01d_workflow_patch_result.json"
out.parent.mkdir(parents=True, exist_ok=True)
payload = {
    "patch_name": "SHADOW_OUTCOME_01D_WORKFLOW_PATCH",
