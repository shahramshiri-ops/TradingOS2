...
PROGRAM = "SIG-E-SHADOW-LANE1B-STATUS-HOTFIX1C-EXTRA-VALIDATION"
...
OUT = Path("outputs/_sig_e_shadow_lane1b_status_hotfix1/sig_e_shadow_lane1b_status_hotfix1c_extra_validation_result.json")
...
def check_one(name, path, errors):
    ...
    if details.get("session_ok") is True and (details.get("alignment_ok") is False or details.get("vol_ok") is False):
        if obj.get("detector_status") == "SESSION_NOT_MATCHED":
            errors.append("%s expected REGIME_NOT_MATCHED, got SESSION_NOT_MATCHED" % name)
...
def main():
    ...
    print("SIG_E_SHADOW_LANE1B_STATUS_HOTFIX1C_EXTRA_VALIDATION_" + result["validation_status"])
    if errors:
        for e in errors:
            print("ERROR:", e)
        raise SystemExit(1)
