...
TOKEN = "scripts/run_sig_shadow_reason_upstream_01.py"
reports = []
...
for wf in WORKFLOWS:
    ...
    if not patched:
        ...
        report["note"] = "patched_after_shadow_ops_or_diag_step"

    backup = wf.with_suffix(wf.suffix + ".bak_reason_upstream_01")
    backup.write_text(original, encoding="utf-8")
    wf.write_text(text, encoding="utf-8")
    report["patched"] = True
    reports.append(report)

out = ROOT / "proofs/reason_upstream_01_workflow_patch_result.json"
out.parent.mkdir(parents=True, exist_ok=True)
payload = {
    "patch_name": "REASON_UPSTREAM_01_WORKFLOW_PATCH",
