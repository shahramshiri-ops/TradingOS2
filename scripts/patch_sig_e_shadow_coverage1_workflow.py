...
PROGRAM = "SIG-E-SHADOW-COVERAGE1-WORKFLOW-PATCH"
OUT = Path("outputs/_sig_e_shadow_coverage1/sig_e_shadow_coverage1_workflow_patch_result.json")
...
def backup(path):
    b = path.with_suffix(path.suffix + ".bak_coverage1")
...
def patch_workflow():
    ...
    if "build_sig_e_shadow_coverage1.py" in text:
        item["reason"] = "ALREADY_PRESENT"
        return item

    block = """          python scripts/build_sig_e_shadow_coverage1.py
          python scripts/validate_sig_e_shadow_coverage1.py

"""

    anchor = "          python scripts/build_sig_e_shadow_observation_report1.py"
    if anchor not in text:
        ...
        # Insert after report validation if present; otherwise after report build.
        anchor2 = "          python scripts/validate_sig_e_shadow_observation_report1.py"
        if anchor2 in text:
    ...
    item["patched"] = True
    item["reason"] = "COVERAGE1_ADDED_TO_WORKFLOW"
    return item
...
def patch_commit():
    ...
    item["patched"] = True
    item["reason"] = "COVERAGE1_ADDED_TO_COMMIT_SAFE_PATHS"
    return item
...
def main():
    ...
    print("SIG_E_SHADOW_COVERAGE1_WORKFLOW_PATCH_" + status)
    for r in results:
