...
def state():
    s=load(STATE)
    if not isinstance(s,dict): s={"state_version":"sig_e_shadow_detector2_obsledger_v1","program":PROGRAM,"created_utc":now(),"detector_id":DETECTOR_ID,"source_spec_id":SOURCE_SPEC_ID,"refresh_records":[],"near_miss_records":[],"shadow_events":[],"outcome_records":[]}
...
def reg_details(d):
    ...
    return {}
def compact(d):
    rg=reg_details(d); obs=observation_run_id()
    return {"observation_run_id":obs,"record_id":obs+"|"+str(d.get("detector_run_id")),"detector_id":d.get("detector_id") or DETECTOR_ID,"source_spec_id":d.get("source_spec_id") or SOURCE_SPEC_ID,"detector_run_id":d.get("detector_run_id"),"created_utc":now(),"detector_created_utc":d.get("created_utc"),"detector_status":d.get("detector_status"),"status_reason":d.get("status_reason"),"is_shadow_match":d.get("is_shadow_match") is True,"is_caveated_observation":d.get("is_caveated_observation") is True,"instrument":d.get("instrument"),"direction":d.get("direction"),"session_bucket":rg.get("session_bucket"),"d1_trend_state":rg.get("d1_trend_state"),"h4_trend_state":rg.get("h4_trend_state"),"htf_alignment":rg.get("htf_alignment"),"range_state":rg.get("range_state"),"h1_bar_open_ts_utc":(d.get("surface_snapshot") or {}).get("h1_bar_open_ts_utc"),"m15_bar_open_ts_utc":(d.get("surface_snapshot") or {}).get("m15_bar_open_ts_utc"),"setup_h1_open_ts_utc":d.get("setup_h1_open_ts_utc"),"trigger_h1_open_ts_utc":d.get("trigger_h1_open_ts_utc"),"shadow_event_id":d.get("shadow_event_id")}
def nm_score(status): return {"REGIME_NOT_MATCHED":1,"SETUP_NOT_FORMED":2,"H1_TRIGGER_WAIT":3,"H1_TRIGGER_NOT_CONFIRMED":3,"M15_NO_FAILURE_POLICY_WAIT":4,"M15_FAILURE_BLOCKED":4,"CAVEATED_SHADOW_MATCH_CONFIRMED":5}.get(status,0)
def main():
    ...
    r=compact(d)
    if r.get("record_id") not in {x.get("record_id") for x in s["refresh_records"] if isinstance(x,dict)}:
        s["refresh_records"].append(r)
    # SESSION_NOT_MATCHED intentionally not a near-miss.
    near={"REGIME_NOT_MATCHED","SETUP_NOT_FORMED","H1_TRIGGER_WAIT","H1_TRIGGER_NOT_CONFIRMED","M15_NO_FAILURE_POLICY_WAIT","M15_FAILURE_BLOCKED","CAVEATED_SHADOW_MATCH_CONFIRMED"}
    if r.get("detector_status") in near:
        n=dict(r); n["near_miss_score"]=nm_score(r.get("detector_status"))
        k=n.get("record_id")+"|"+str(n.get("detector_status"))
        ex={str(x.get("record_id"))+"|"+str(x.get("detector_status")) for x in s["near_miss_records"] if isinstance(x,dict)}
        if k not in ex: s["near_miss_records"].append(n)
    if d.get("detector_status")=="CAVEATED_SHADOW_MATCH_CONFIRMED" and d.get("shadow_event_id"):
        if d.get("shadow_event_id") not in {e.get("shadow_event_id") for e in s["shadow_events"] if isinstance(e,dict)}:
            s["shadow_events"].append({"detector_id":DETECTOR_ID,"source_spec_id":SOURCE_SPEC_ID,"shadow_event_id":d.get("shadow_event_id"),"created_utc":d.get("created_utc"),"instrument":"USDJPY","direction":"SHORT","classification":"CAVEATED_OBSERVATION_ONLY","detector_status":d.get("detector_status"),"setup_h1_open_ts_utc":d.get("setup_h1_open_ts_utc"),"trigger_h1_open_ts_utc":d.get("trigger_h1_open_ts_utc"),"observation_horizon_h1_bars":8,"outcome_status":"PENDING","authority":AUTH,"boundary":BOUND})
    ...
    write(STATE,s); write(RUNTIME,cur); write(PANEL,cur); write(OUT,{"program":"SIG-E-SHADOW-DETECTOR2-OBSLEDGER1-PERSIST1","created_utc":now(),"build_status":"PASS","summary":cur["summary"],"authority":AUTH,"boundary":BOUND})
    print("SIG_E_SHADOW_DETECTOR2_OBSLEDGER1_PERSIST1_DONE")
    print("REFRESH_COUNT_TOTAL="+str(cur["summary"].get("refresh_count_total")))
