...
if not js.exists():
    ...
    for token in ["BRAIN4_UI_OPS_01A", "function addClasses", "classList.add(...clean)", "UI-only patch"]:
        if token not in jst:
            failures.append(f"brain4_ui_ops_01.js missing token: {token}")
    if "classList.add(className)" in jst:
        failures.append("old classList.add(className) bug still present")

if not css.exists():
    failures.append("missing brain4_ui_ops_01.css")

result = {
    "validation_name": "BRAIN4_UI_OPS_01A_INSTALL_VALIDATION",
...
out = ROOT / "proofs/brain4_ui_ops_01a_install_validation_result.json"
